...
<?php
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				// start of diagnosis
				$get_medical_rs2 = $this->nurse_model->get_doctor_notes($patient_id,$visit_id1);
				$notes_num_rows = count($get_medical_rs2);
				//echo $num_rows;
				
				if($notes_num_rows > 0){
					foreach ($get_medical_rs2 as $notes_key):
						$doctor_notes_id = $notes_key->doctor_notes_id;
						$doctor_notes = $notes_key->doctor_notes;
						$notes .='<span class="bold">Other Notes: </span> '.$doctor_notes.'<br>';
						
					endforeach;
				}
				else
				{
					$notes .= '<span class="bold">Other Notes: </span> -';
				}
				// end of diagnosis
				$result .= 
					'
