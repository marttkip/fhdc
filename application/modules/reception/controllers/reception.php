...
{	
	...
	*/
	public function set_visit($primary_key)
	{
		...
		$insurance_company = $this->reception_model->get_patient_insurance_company($primary_key);
		...
		$v_data['patient'] = 'Surname: <span style="font-weight: normal;">'.$patient_surname.'</span> Othernames: <span style="font-weight: normal;">'.$patient_othernames.'</span> Insurance Company: <span style="font-weight: normal;">'.$insurance_company.'</span> Account Balance : <span style="font-weight: normal;">'.$account_balance.'</span> <a href="'.site_url().'/administration/individual_statement/'.$primary_key.'/2" class="btn btn-sm btn-primary" target="_blank" style="margin-top: 5px;">Patient Statement</a>';
	...
	public function save_visit($patient_id)
	{
		...
		if(isset($_POST['department_id'])){
			...
			else if($_POST['department_id'] == 10)
			{
				//if nurse visit doctor must be selected
				$this->form_validation->set_rules('personnel_id', 'Doctor', 'required|is_natural_no_zero');
				$this->form_validation->set_rules('service_charge_name', 'Consultation Type', 'required|is_natural_no_zero');
		...
		else
		{
			...
			else
			{
				...
				if($_POST['department_id'] == 7 || $_POST['department_id'] == 12 || $_POST['department_id'] == 10)
				{
