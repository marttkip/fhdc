...
{	
	...
	public function patient_card($visit_id, $mike = NULL, $module = NULL,$opener = NULL)
	{
		...
		$v_data['patient'] = 'Surname: <span style="font-weight: normal;">'.$patient_surname.'</span> Othernames: <span style="font-weight: normal;">'.$patient_othernames.'</span> Age: <span style="font-weight: normal;">'.$age.'</span> Gender: <span style="font-weight: normal;">'.$gender.'</span> Insurance Company: <span style="font-weight: normal;">'.$insurance_company.'</span>';
	...
	public function save_doctor_notes($visit_id){
		...
		$patient_id = $this->nurse_model->get_patient_id($visit_id);
		$rs = $this->nurse_model->get_doctor_notes($patient_id,$visit_id);
		$num_doc_notes = count($rs);
		
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'patient_id'=>$patient_id,'doctor_notes'=>$notes);
			$this->db->insert('doctor_patient_notes', $visit_data);

		}
		else {
			$visit_data = array('doctor_notes'=>$notes);
			$this->db->where('patient_id = '.$patient_id.' AND visit_id = '.$visit_id);
			$this->db->update('doctor_patient_notes', $visit_data);
		}
	...
	public function search_visit_patients($module = NULL)
	{
		...
		$search = $visit_type_id.$surname.$other_name;
	...
	*/
	...
	public function close_patient_treatment_search($module)
	{
		$this->session->unset_userdata('patient_treatment_statement_search');
	...
	//  new things starting
	public function save_plan($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'plan_description'=>$notes);
			$this->db->insert('visit_plan', $visit_data);

		}
		else {
			$visit_data = array('plan_description'=>$notes);
	...
	public function save_investigation($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'investigation'=>$notes);
			$this->db->insert('visit_investigations', $visit_data);

		}
		else {
			$visit_data = array('investigation'=>$notes);
			$this->db->where('visit_id = '.$visit_id);
	...
	public function save_oc($visit_id){


		$filled=$this->input->post('filled');
		$missing=$this->input->post('missing');
		$decayed=$this->input->post('decayed');
		$soft_tissue=$this->input->post('soft_tissue');
	...
	public function save_histories($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'past_medical_history'=>$notes, 'past_dental_history'=>$notes2);
			$this->db->insert('visit_history', $visit_data);

		}
		else {
			$visit_data = array('past_medical_history'=>$notes, 'past_dental_history'=>$notes2);
			$this->db->where('visit_id = '.$visit_id);
	...
	public function save_findings($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'finding_description'=>$notes);
			$this->db->insert('visit_finding', $visit_data);

		}
		else {
			$visit_data = array('finding_description'=>$notes);
			$this->db->where('visit_id = '.$visit_id);
	...
	public function save_rx($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'rx_description'=>$notes);
			$this->db->insert('visit_rx', $visit_data);

		}
		else {
			$visit_data = array('rx_description'=>$notes);
			$this->db->where('visit_id = '.$visit_id);
	...
	public function save_hpco($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'hpco_description'=>$notes);
			$this->db->insert('visit_hpco', $visit_data);

		}
		else {
			$visit_data = array('hpco_description'=>$notes);
			$this->db->where('visit_id = '.$visit_id);
	...
	public function save_tca($visit_id){
		...
		if($num_doc_notes == 0){	
			$visit_data = array('visit_id'=>$visit_id,'tca_description'=>$notes);
			$this->db->insert('visit_tca', $visit_data);

		}
		else {
			$visit_data = array('tca_description'=>$notes);
			$this->db->where('visit_id = '.$visit_id);
	...
	{
		$notes=$this->input->post('todays_notes'.$visit_id);
		// var_dump($_POST);die();
