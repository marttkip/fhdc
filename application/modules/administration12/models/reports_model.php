...
class Reports_model extends CI_Model 
{
		...
		if($visits_query->num_rows() > 0)
		{
			...
			*/
			...
			foreach($visits_query->result() as $row)
			{
				...
				if($row->insurance_company_id > 0 )
				{
					$insurance_company_query = $this->get_insurance_company_name($row->insurance_company_id);
					
					foreach($insurance_company_query->result() as $row_two)
					{
						
						$insurance_company_name = $row_two->insurance_company_name;
					}
				}
				else
				{
				...
				if($debtors == 'true' && ($balance > 0))
				{
					...
					$report[$row_count][3] = $insurance_company_name;
					$report[$row_count][4] = $doctor;
					$current_column = 5;
	...
	*/
	...
	public function get_insurance_company_name($insurance_company_id)
	{
		//invoiced
		$this->db->from('insurance_company');
		$this->db->select('*');
